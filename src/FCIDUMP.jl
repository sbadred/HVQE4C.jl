...
function Generate_FCIDUMP(molecule, basis,name)
    ...
    def Molecules(molecule,basis,name):
        mol =pyscf.gto.M(
                     atom = molecule,
                     basis =basis,charge=0,spin=0,verbose=3)
        myhf = pyscf.scf.RHF(mol)
        myhf.kernel()
        c = myhf.mo_coeff
        h1e=functools.reduce(np.dot, (np.transpose(c), myhf.get_hcore(), c))
        eri = pyscf.ao2mo.kernel(mol, c,aosym="s1")
        tools.fcidump.from_integrals(name, h1e, eri, c.shape[1],
                                  mol.nelectron,nuc=mol.energy_nuc(), ms=0,tol=1e-12)
        return myhf.e_tot
    """
     return py"Molecules"(molecule,basis,name)
end

    # """
    # Generate_FCIDUMP: generate fcidump file containing molecular integrals in the active space 
    # """
    # function Generate_FCIDUMPCAS(molecule, basis,name,ncas,nelecas)
    #     mol =pyscf.gto.M(
    #         atom = molecule,
    #         basis =basis,
    #     )
    #     myhf = pyscf.scf.RHF(mol)
    #     myhf.kernel()
    #     mc = pyscf.mcscf.CASCI(myhf, ncas, nelecas)
    #     tools.fcidump.from_integrals(name, mc.get_h1eff()[1], mc.get_h2eff(), mc.ncas, mc.nelecas,ms=0,tol=1e-12,nuc=mc.get_h1eff()[2]) 
    # end
#tol is the truncation tolerance for the values of integrals 
